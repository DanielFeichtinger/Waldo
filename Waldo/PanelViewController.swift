...
class PanelViewController: NSViewController, WKNavigationDelegate, WKScriptMessageHandler, WKUIDelegate {
    ...
    override func viewDidAppear() {
        if isActive { delegate!.centreActivePanel() }
    }
    
    @IBAction func destroyPanel(_ sender: Any) {
        delegate.removePanel(self)
    }
    
    @IBAction func navBack(_ sender: Any) {
        if panel.currentVisit!.previous != nil {
            let previous = panel.currentVisit!.previous!
            panel.currentVisit = previous
            navigate(previous)
            save()
        }
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        if !isShuttingDown && webView.title != "" {
            isLoading = false
            panel.currentVisit?.updateResource(url: webView.url!, title: webView.title!, context: managedContext!)
            save()
            intentText = nil
            redrawSecurityBar()
        }
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        if navigationAction.targetFrame == nil || navigationAction.targetFrame!.isMainFrame {
            ...
            if shouldCreateNewVisit(navigationType: navigationAction.navigationType) {
                visit = Visit.init(navigationAction: navigationAction, intentText: intentText, previous: panel.currentVisit!, context: managedContext!)
            } else {
                visit = panel.currentVisit
            }
            if navigationAction.modifierFlags.contains(.command) {
                decisionHandler(.cancel)
                intentText = nil
                delegate!.addPanel(visit, from: view)
            } else {
                decisionHandler(.allow)
                panel.currentVisit = visit
    ...
    func webView(_ webView: WKWebView, createWebViewWith configuration: WKWebViewConfiguration, for navigationAction: WKNavigationAction, windowFeatures: WKWindowFeatures) -> WKWebView? {
        ...
        let visit = Visit.init(navigationAction: navigationAction, intentText: intentText, previous: panel.currentVisit!, context: managedContext!)
        if navigationAction.modifierFlags.contains(.command) {
            delegate.addPanel(visit, from: view, withWebView: newWebView)
        } else {
            panel.currentVisit = visit
    ...
    func loadResource() {
        if panel.currentVisit != nil {
            intentType = panel.currentVisit?.intentType
            webView.load(panel.currentVisit!.urlRequest! as URLRequest)
            drawLoadingState()
        }
    }
    ...
    private func redrawSecurityBar() {
        ...
        if panel.currentVisit!.resource != nil {
            visitCountLabel.stringValue = "\(panel.currentVisit!.resource!.visitCount)"
        }
    }
    
    private func drawLoadingState() {
        intentLabel.stringValue = panel.currentVisit!.displayText
    }
    ...
    override func mouseDown(with event: NSEvent) {
        delegate!.changeActivePanel(self)
    }
    
    func resizeWidth(_ delta: CGFloat) {
        let newWidth = widthConstraint.constant + delta
        if (delta < 0 && newWidth >= 250.0) ||
           (delta > 0 && newWidth <= delegate!.maxPanelWidth)  {
            widthConstraint.constant = newWidth
            panel.width = Float.init(newWidth)
        }
    }
    
    func persistWidth() {
        save()
        UserDefaults.standard.set(panel.width, forKey: "PanelWidth")
    }
